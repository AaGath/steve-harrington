...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        FIND[query] = {"results": result, "total_len": len_results, "max_pages": max_pages} # TrojanzHex's Idea Of Dicts✈︎
        ...
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"⌨︎ Page 1/{len_result if len_result < max_pages else max_pages} ⌨︎", callback_data="ignore")
        ])
        ...
        if show_invite:
            ...
            for y in INVITE_LINK.get(str(group_id)):
                ...
                if ((len(ibuttons)%2) == 0):
                    ibuttons.append(
                        [
                            InlineKeyboardButton(f"☠︎︎ {chat_name} ☠︎︎", url=invite_link)
                        ]
                    )
